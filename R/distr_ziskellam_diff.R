...
distr_ziskellam_diff_density <- function(y, f) {
  ...
  r2 <- f[, 2, drop = FALSE]
  p <- f[, 3, drop = FALSE]
  res_density <- suppressWarnings((y == 0L) * p + (1 - p) * exp(-(r1 + r2)) * (r1 / r2)^(y / 2) * besselI(x = 2 * sqrt(r1 * r2), nu = y))
...
distr_ziskellam_diff_loglik <- function(y, f) {
  ...
  res_loglik[y != 0L, ] <- suppressWarnings(log(1 - p[y != 0L, ]) + y[y != 0L, ] / 2 * log(r1[y != 0L, ] / r2[y != 0L, ]) - r1[y != 0L, ] - r2[y != 0L, ] + log(besselI(x = 2 * sqrt(r1[y != 0L, ] * r2[y != 0L, ]), nu = y[y != 0L, ])))
  res_loglik[y == 0L, ] <- suppressWarnings(log(p[y == 0L, ] + (1 - p[y == 0L, ]) * exp(-(r1[y == 0L, ] + r2[y == 0L, ])) * besselI(x = 2 * sqrt(r1[y == 0L, ] * r2[y == 0L, ]), nu = 0)))
...
distr_ziskellam_diff_random <- function(t, f) {
  ...
  res_random[is.na(res_random)] <- suppressWarnings(stats::rpois(sum(is.na(res_random)), lambda = r1) - stats::rpois(sum(is.na(res_random)), lambda = r2))
