...
# BETA DISTRIBUTION / SHAPE PARAMETRIZATION


# Parameters Function ----------------------------------------------------------
distr_beta_shape_parameters <- function(n) {
  group_of_par_names <- c("shape", "shape")
  par_names <- c("shape", "shape")
...
# Density Function -------------------------------------------------------------
distr_beta_shape_density <- function(y, f) {
  t <- nrow(f)
  a <- f[, 1, drop = FALSE]
  b <- f[, 2, drop = FALSE]
...
distr_beta_shape_loglik <- function(y, f) {
  t <- nrow(f)
  a <- f[, 1, drop = FALSE]
  b <- f[, 2, drop = FALSE]
...
# Mean Function ----------------------------------------------------------------
distr_beta_shape_mean <- function(f) {
  t <- nrow(f)
  a <- f[, 1, drop = FALSE]
  b <- f[, 2, drop = FALSE]
  res_mean <- a / (a + b)
...
# Variance Function ------------------------------------------------------------
distr_beta_shape_var <- function(f) {
  t <- nrow(f)
  a <- f[, 1, drop = FALSE]
  b <- f[, 2, drop = FALSE]
...
# Score Function ---------------------------------------------------------------
distr_beta_shape_score <- function(y, f) {
  t <- nrow(f)
  a <- f[, 1, drop = FALSE]
  b <- f[, 2, drop = FALSE]
...
distr_beta_shape_fisher <- function(f) {
  t <- nrow(f)
  a <- f[, 1, drop = FALSE]
  b <- f[, 2, drop = FALSE]
...
distr_beta_shape_random <- function(t, f) {
  a <- f[1]
  b <- f[2]
...
distr_beta_shape_start <- function(y) {
