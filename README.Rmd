...
You can install the development version of gasmodel from [GitHub](https://github.com/) with:
...
The key function is `gas()` which estimates GAS models.
Additional functions include `gas_simulate()` which simulates GAS models, `gas_forecast()` which forecasts GAS models, `gas_filter()` which obtains filtered time-varying parameters of GAS models, and `gas_bootstrap()` which bootstraps coefficients of GAS models.

The list of supported distributions can be obtained by `distr()`.
The functions working with distributions are `distr_density()` which computes the density, `distr_mean()` which computes the mean, `distr_var()` which computes the variance, `distr_score()` which computes the score, `distr_fisher()` which computes the Fisher information, and `distr_random()` which generates random observations.

The included datasets are `bookshop_sales` and `ice_hockey_championships`.
