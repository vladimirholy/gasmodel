...
Model estimation is performed by the maximum likelihood method.
...
As a simple example, let us model daily toilet paper sales in a store. We estimate the GAS model based on the negative binomial distribution with time-varying mean and incorporating dummy variables to indicate the day of the week and whether the product is being promoted. We also plot the filtered time-varying parameters:
...
``` {r distr, echo = FALSE, message = FALSE, warning = FALSE}
library("dplyr")
...
distr_table <- distr() %>%
  ...
  ungroup() %>%
  mutate(dim = recode(dim, "uni" = "Univariate", "multi" = "Multivariate")) %>%
  ...
  rename(c("Label" = "distr", "Distribution" = "distr_title", "Dimension" = "dim", "Data Type" = "type", "Parametrizations" = "param")) %>%
  arrange(Label)

knitr::kable(distr_table)
```
...
More recently, a variety of novel score-driven models has been proposed, such as the Beta-t-(E)GARCH model of Harvey and Chakravarty (2008), a Skellam model of Koopman et al. (2018), a directional model of Harvey (2019), a bivariate Poisson model of Koopman and Lit (2019), and a ranking model of Holý and Zouhar (2022).
For an overview of various GAS models, see Harvey (2022).

The extensive GAS literature is listed on [www.gasmodel.com](https://www.gasmodel.com).
